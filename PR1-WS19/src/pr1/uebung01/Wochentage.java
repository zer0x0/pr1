...
import static pr.MakeItSimple.*; 		// Importieren von Custom Methode
...
public class Wochentage{				// Name der Methode - Name des Programms
	...
	while (bool_all==true) {
		...
		if ((v_month==2) && !((v_year%4==0) && ((v_year%100!=0 || v_year%400==0))) && (v_day>28)) {
			println("In diesem Jahr hat Februar nur 28 Tage, bitte Datum neu eingeben");
			bool_day = true;
			bool_month = true;
			bool_year = true;}
			else
		if ((v_month==4 || v_month==6 || v_month==9 || v_month==11) && (v_day>30)) {
